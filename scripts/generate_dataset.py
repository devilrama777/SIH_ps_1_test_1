...
def generate_dataset():
    ...
    # 2. Exit / Sleep (Termination words)
    print("Synthesizing 'Exit' / 'Sleep' samples...")
    exit_words = ["Exit", "exit", "Sleep", "sleep", "Aura sleep", "Go to sleep", "Shut down", "Stop listening"]
    count_exit = 0
    for v_item in selected_voices:
        for rate in [-2, 0, 2]:
            for vol in [80, 100]:
                for w in exit_words:
                    ...
                    for aug_i in range(3):
                        ...
                        save_wav(augmented, dst)
                    count_exit += 1
    print(f"Generated {count_exit * 3} 'Exit'/'Sleep' samples.")

    # 3. Unknown (Large, varied vocabulary including natural questions!)
    print("Synthesizing diverse 'Unknown' conversational samples...")
    unknown_phrases = [
        # Questions users naturally ask:
        ...
        "tell me the speed", "is the system ready", "check the connection",
        # Common single words:
        ...
        "radio", "yes", "no", "okay", "start", "stop", "next", "previous", "cancel",
        # Numbers:
    ...
    # 4. Silence (Realistic quiet room noise levels)
    print("Synthesizing 'Silence' samples...")
    for i in range(350):
        # Realistic quiet room mic levels (RMS ~0.001 - 0.015)
        scale = np.random.uniform(0.0005, 0.015)
        if i % 2 == 0:
            noise = np.random.normal(0, scale, NUM_SAMPLES)
        else:
            t = np.linspace(0, CLIP_DURATION, NUM_SAMPLES, endpoint=False)
            noise = (scale * 0.5) * np.sin(2 * np.pi * 50 * t) + np.random.normal(0, scale * 0.5, NUM_SAMPLES)
        dst = os.path.join(output_base, 'silence', f"silence_{i}.wav")
        save_wav(noise.astype(np.float32), dst)
    print(f"Generated 350 'Silence' samples.")
